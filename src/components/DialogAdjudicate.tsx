...
import {adjudicate} from "@/service/Dispute.tsx";
import {toast} from "@/components/ui/use-toast.ts";

export function DialogAdjudicate({isDialogOpen, setIsDialogOpen, disputeID, onResponse}: {
	...
	const onSubmit = async () => {
		try {
			setLoading(true);
			const response = await adjudicate(disputeID);
			if (response) {
	...
	return (
		<Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
			<DialogContent className="sm:max-w-[625px]">
				<DialogHeader>
					<DialogTitle>Adjudicate</DialogTitle>
					<DialogDescription>
						[{disputeID}] Resolves the case in the customer's favor.
					</DialogDescription>
				</DialogHeader>
				<DialogFooter>
					{loading ? (
